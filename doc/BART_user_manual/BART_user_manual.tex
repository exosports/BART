...
\item Ryan Challener, University of Central Florida (email).
...
(\href{https://github.com/joeharr4/BART}{BART}) project combines the
radiative-transfer code
(\href{https://github.com/pcubillos/transit}{Transit}) with the
...
diversity of project, not necessarily linked to exoplanet atmospheric
retrieval.
...
\begin{figure}[htb]
\centerline{
\includegraphics[width=0.9\textwidth, clip]{figs/BARTcharts.ps}}
...
The BART package is organized as follow: \newline
...
\noindent -- GSL \findme{Give specifics}. \\

\subsection{Install and Compile}
\label{sec:install}

To obtain the latest stable version from the BART
\href{https://github.com/joeharr4/BART/releases}{releases} page
...
unzip 06_hit08.zip
\end{verbatim}}
\findme{Verbatim doesn't break lines.} \\
...
calculations.  This file can take command-line arguments or a
configuration file (preferred) to set up the calculation details.

The BART configuration file is the one that set up and determine the inputs.
The BART command-line arguments are roughly divided in three main
subgroups.

Some other input files that either exist before the run or can be
created during the run.
The BART program is capable of producing most of the inputs.
...
The BART configuration file is the main file that set 

The main file for BART program is the BART configuration file.

The BART program uses a Configuration File were the user set up
the arguments.  Note that some of these argument inputs are files that
should exist before running BART.

\findme{Speak about the folder structure}.

\subsection{Transit-Line Information (TLI) File}
\label{sec:TLI}
(created by the Pylineread routine placed in the BART/modules/transit/
sub-module)

   To make a transit linelist input (TLI) file do the following:

        - cd to pylineread module
                cd BART/modules/transit/pylineread

        - inside the pylineread directory, make a TLI sub-directory
                mkdir TLI
        ...
        - copy the pylineread example configuration file
          ../examples/pyline\_example.cfg to the TLI directory
                cp ../examples/pyline\_example.cfg pyline\_run01.cfg

        - read the configuration file header and download desired databases
          with the partition functions from the provided links

        - edit the configuration file with the following information:
                a) paths to the databases
                b) type of the input databases
                c) desired output TLI filename
                d) desired initial and final wavelength range in microns
                e) desired verbosity level

        - run the pylineread module as:
                ../src/pylineread.py -c pyline\_run01.cfg

        - running the pylineread module will produce the TLI file with the
          name requested in the configuration file. The file will be placed
          in the current working directory (TLI/) if not specified, or in the
          directory specified
...
\label{sec:CS}
 (provided in the 
   BART/modules/transit/inputs/ directory)

\subsection{Transiting Extrasolar Planet (TEP) File}
\label{sec:TEP}
 containing planetary system information (some tepfiles
   are provided in the BART/inputs/tep/ directory)

\subsection{Stellar Spectrum File}
\label{sec:stellarSpec}
Kurucz stellar spectrum file (some kurucz files are provided
   in BART/inputs/kurucz/ directory)

\subsection{Elemental Abundances File}
\label{sec:elemAbund}
 (abundances\_Asplund2009.txt file is
   provided in BART/inputs/ directory)
\subsection{Filter Files}
\label{sec:filters}
  (some Spitzer filter files are provided in BART/inputs/
   filters/ directory)
...
TEA produces one output file:
    1. <atmopsheric\_file>.tea, this file is placed in
       and the BART modified version is placed in the
       BART output folder

MCcubed produces one output file:
    1. output.npy - a binary file containing 3D array of
                    parameters per each chain and each iteration

TRANSIT produces three output files:
    1. \_toomuch.dat - list of radii at each wavelength sample
                      where the optical depth reaches the maximum
                      'toomuch' value

    2. \_sampling.dat - provides sampling information 

    3. \_spectrum.dat - gives the flux/modulation (eclipse/transit 
                       geometry) vs. wavelength

In addition BART produces the following:
    1. output file:
        - band\_eclipse.npy - a binary file containing 3D array of
                         eclipse depths per each chain and each iteration
    2. plots:
        - best fitting T-P profile with boundaries
        - best fitting abundances plot
...
Edit the file with the correct information:
  ...
  1. Give paths to the input tep and kurucz files and the output
     directory

  2. ::: Atmospheric pressure layers :::
     Provide the information needed to make a pressure file with desired
     number of layers, range and scale.

  3. ::: Elemental-abundances file :::
  ...
  5. ::: Atmospheric Elemental Abundances (pre-atmospheric) File :::
  ...
  6. ::: Atmospheric File (P, T, species-abundances)  :::
  ...
  7. ::: MCMC arguments  :::
     Provide information to run ../modules/mccubed/ module:
       ...
       - 3-element tuple to locate the fitting function
         (<function\_name> <routine\_name> <path\_to\_routine>)
       - list of molecules that have line list data
       - temperature boundaries (this information depends on the
         line-list and CS databases temperature range)
       - model fitting parameters (temperature-pressure and abundances
         scale factors).
       - set basic MCMC variables and define the filename to store
         model fit
       - set desired verbosity level

  8. ::: Transit variables  :::
     Provide information to run ../modules/transit module:
       - set the desired name of the ../modules/transit/ module
         configuration file that will be produced based on the
         information provided in this section
       - locate TRANSIT line information and CS files
       - spectrum information (Note: The range given must be within
         the wavelength range given for the TLI file creation)
       - the desired geometry solution (transit or eclipse),
       ...
       - information to make an opacity-grid file.
         a) If variable values and new file name is provided,
         ...
         b) If the opacity file already exists give the path to it.
         ...
         c) If opacityfile name is commented, the opacity calculation
...
\item Determine what observing geometry you will test, set {\tttb solution} to eclipse or transit.  Based on that, search for the available data (eclipse or transit depth, respectively) from the literature or your own data analysis.  Put those values in the {\tttb data}, and {\tttb uncert} arguments.
...
or create a new one.  If the former, set {\tttb atmfile} to an existing
file (and skip to step (\ref{item:molfit})).

\item Set the atmospheric pressure layers with the {\tttb press\_file}, 
{\tttb n\_layers},  {\tttb p\_top}, {\tttb p\_bottom}, and {\tttb log}
arguments.

\item Set the initial atmospheric temperature profile.  Set with {\tttm
PTtype} if you want to use a model as in
\citet{LineEtal2013apjRetrievalI} or 
\citet{MadhusudhanSeager2009ApJ-AbundanceMethod}.  Adjust the model
parameters in {\tttb PTinit} to set the temperature profile
(See Section \ref{sec:PT}).  What is an appropriate initial temperature
profile? that is a question yet to be answered.  Having no prior
knowledge on the planet's properties, temperatures around the
equilibrium temperature would be an initial guess.
As a help, run the scripts in
\href{https://github.com/joeharr4/BART/blob/master/scripts/quickguide.py}
{BART/scripts/quickguide.py} to see how the temperature profile looks
...
See the transit documentation (transit/doc/transitUM.pdf) to learn more
about all the Transit settings.  It’s a bit outdated, but most of the
info is current.
...
This are quick guidelines to generate a TLI file with the Transit's
pylineread module.  For a detailed description see the Transit user's
manual \findme{Link to TUM}.

cd into the {\tttb myBART/} and create a directory to run pylineread
and place the TLI file: \newline
{\tttb mkdir TLI}  \\
{\tttb cd TLI}     \\

Copy a pylineread configuration-file template: \newline
{\tttb cp ../modules/transit/pylineread/examples/pyline\_example.cfg ./pyline.cfg} \\

Edit the configuration file and run it: \newline
{\tttb ../modules/transit/pylineread/src/pylineread.py -c ./pyline.cfg}  \\
