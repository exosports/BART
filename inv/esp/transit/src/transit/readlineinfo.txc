  ...
  */
  ...
  while(1){
    ...
    //If binary read a data structure
      ...
      rn=fread(&res[i].wl,sizeof(PREC_LNDATA),1,fp);
      rn+=fread(&res[i].isoid,sizeof(short),1,fp);
      rn+=fread(&res[i].elow,sizeof(PREC_LNDATA),1,fp);
      rn+=fread(&res[i].gf,sizeof(PREC_LNDATA),1,fp);
