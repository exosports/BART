...
int main(int argc, char *argv[])
{
  ...
  Pprintf(1,"Welcome!\n");
  Pprintf(2,"Lower wavelength (nm)[%g]?: ",wl1);
  if(Pgetf(0,&tf1,10)>0)
    wl1=tf1;

  Pprintf(2,"Upper wavelength (nm)[%g]?: ",wl2);
  if(Pgetf(0,&tf1,10)>0)
    wl2=tf1;
  ...
  Pprintf(1,"Done reading the file.\n\ndbread_pands() test results:\n"
  ...
  */
  ...
  while(Pgeti(0,&ans,(int)(log10(n))+1)>=0){
    if(ans<n&&ans>=0){
      lp=lines+ans;
      Pprintf(1,"Record Position: %i\nWavelength: %.10g\n"
	      ,lp->recpos,lp->wl);
      Pprintf(1,"Lower Energy Level: %.10g\nLog(gf): %.10g\n"
	      , lp->elow, lp->lgf);
      Pprintf(1,"Isotope Name: %s\n"
	      ,isotope[lp->isoid]);
    }
    else
