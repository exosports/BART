...
def initialPT(date_dir, tepfile, press_file, a1, a2, p1, p3, T3_fac):
  """
  Generate a non-inverted temperature profile using the parameteried model
  described in Madhusudhan & Seager (2009).  Plot the profile to screen and
  save the figure in the specified directory.  The generated PT profile is a
  semi-adiabatic profile with a bottom-layer temperature similar to the
  ...
  T3_fac: Float
     ...
     Empirically determined to be between (1, 1.5) in that way accounting
     for the possible spectral features (FINDME: Get reference).
  ...
  Revisions:
  ----------
  2014-04-08  Jasmina   Initial version
  2014-07-02  Jasmina   Plots are automatically placed in the plots/ directory
